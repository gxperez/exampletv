...
datatransformer.addVisual("echart-pie", 	
{
	...
	subtitle: 		{label: "subtitle",  type: String,  required: true },
	group: 			{label: "group",  	 type: datatransformer.typeGroup,  required: true},
	measure: 		{label: "measure",   type: datatransformer.typeMeasure,  required: true},
	groupFormula: 	{label: "formula",   type: datatransformer.typeEnum, values: _formulas,  required: true},
	theme: 			{label: "theme",   	 type: datatransformer.typeEnum, values: _themeList,  required: true}
},
function(){
	this.render = function(){
			var _echartObj = echarts.init(document.getElementById(this.config.elemId)),
				_data = this.data.transformed,
				_measureObj = {},
				_dataOptionsObj = {},
				_generateData = [],
				_dataForPie = [],
			...
			_measureObj[this.config.measure] = this.config.groupFormula+"( "+this.util.generateMeasureColumn(this.config.measure)+" )";
			...
			for(var d in _generateData){
				var _value = _generateData[d][this.config.measure];
...
datatransformer.addVisual("echart-bar", 	
{
	...
	subtitle: 		{label: "subtitle",  type: String,  required: true},
	group: 			{label: "group",  	 type: datatransformer.typeGroup,  required: true},
	measures: 		{label: "measures",  type: datatransformer.typeMultipleMeasures,  required: true},
	groupFormula: 	{label: "formula",   type: datatransformer.typeEnum, values: _formulas,  required: true},
	theme: 			{label: "theme",   	 type: datatransformer.typeEnum, values: _themeList,  required: true},
	horizontal: 	{label: "horizontal", type: Boolean,  required: true},
},
function(){
	this.render = function(){
			var _echartObj = echarts.init(document.getElementById(this.config.elemId)),
			...
			{
				_measuresObj[this.config.measures[m]] = this.config.groupFormula+"( "+this.util.generateMeasureColumn(this.config.measures[m])+" )";	
			...
			for(var m in this.config.measures){
				...
				for (var c in _categories)
				{
					...
					var _value = _generateData.filter(function(x){
...
datatransformer.addVisual("echart-line", 	
{
	...
	groupFormula: 	{label: "formula",   type: datatransformer.typeEnum, values: _formulas,  required: true},
	theme: 			{label: "theme",   	 type: datatransformer.typeEnum, values: _themeList,  required: true},
	shadow:         {label: "shadow",  type: Boolean,  required: true}
},
function(){
	this.render = function(){
			var _echartObj = echarts.init(document.getElementById(this.config.elemId)),
...
datatransformer.addVisual("echart-gauge-basic", 	
{
	...
	subtitle: 		{label: "subtitle",  	 type: String,  required: true},
	group: 			{label: "group",  	 	 type: datatransformer.typeGroup,  required: true},
	filter: 		{label: "filter group",  type: String,  required: true},
	measure: 		{label: "measure", 	 	 type: datatransformer.typeMeasure,  required: true},
...
function(){
	this.render = function(){
			...
			_measureObj[this.config.measure] = "max ( "+this.util.generateMeasureColumn(this.config.measure)+" )";
			...
			_dataForGauge.push(
			{
				value: _generateData[0][this.config.measure] || 0,
				name: _filter
			});
...
datatransformer.addVisual("echart-gauge-advanced", 	
{
	...
	group: 			{label: "group",  	 	 			type: datatransformer.typeGroup,  required: true},
	filter: 		{label: "filter group",  			type: String,  required: true},
	measure: 		{label: "measures principal", 	 	type: datatransformer.typeMeasure,  required: true},
	measures: 		{label: "measures categories", 	 	type: datatransformer.typeMultipleMeasures,  required: true},
	theme: 			{label: "theme",   	 	 			type: datatransformer.typeEnum, values: _themeList,  required: true},
	colors: 		{label: "color (red,#ff4500)", 	 	type: String},
	split:          {label: "split",  type: Number,  required: true},
	horizontal:     {label: "horizontal",  type: Boolean},
	horRadius: 		{label: "horizontal radius",  type: Number},
},
function(){
	this.render = function(){
			var _echartObj = echarts.init(document.getElementById(this.config.elemId)),
			...
			function _getSeparetor(avgNumber,topNumber){
				...
				while(_util.round(_o[idx],0) <  topNumber)
				{
					_o.push(_util.round(_o[idx] + avgNumber,14));
					idx++;
				}

				_o[_o.length-1] = _util.round(_o[_o.length-1],0);
			...
			_dataForGauge.push(
			{
				value: _generateData[0][this.config.measure] || 0,  //_generateData.filter(function(x){ return x[_group] == _filter})[0][this.config.measure] || 0, 
				name: _filter
			});
			...
			for(var m in this.config.measures)
			{
				_measuresObj[this.config.measures[m]] = "max ( "+this.util.generateMeasureColumn(this.config.measures[m])+" )";	
				_leyends.push(this.config.measures[m]);
			}
			...
			for(var m in this.config.measures)
			{
				...
				var _closestNumber = this.util.getClosestNumber(_cValue,_gaugeNumberSeparetor)
