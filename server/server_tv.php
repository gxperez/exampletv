...
function wsOnActivar($clientID, $message, $messageLength, $binary) {
...
global $Server;
	...
	//The speaker is the only person in the room. Don't let them feel lonely.
	if ( sizeof($Server->wsClients) == 1 )
		$Server->wsSend($clientID, "Estas Solo. Eres el Primer TV");
	else
		//Send the message to everyone but the person who said it
		foreach ( $Server->wsClients as $id => $client )
			if ( $id != $clientID )
				$Server->wsSend($id, "EL Mac Adress TV: \"$message\"");
...
$Server->bind('activar', 'wsOnActivar');
